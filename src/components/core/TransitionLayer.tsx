...
export default function TransitionLayer({ background = 'bg-secondary-200' }: BackgroundColors) {
    return (
        <div className='absolute top-0 left-0 right-0 z-10 flex flex-col'>
            ...
            <motion.div className='flex' initial={{ justifyItems: 'start' }} exit={{ justifyContent: 'end' }}>
                <motion.div className={`bg-white h-[100dvh] brightness-[95%]`} initial={{ width: '120%' }} animate={{ width: '0%' }} exit={{ width: '120%' }} transition={{ delay: 0.4, duration: 0.5, ease: 'easeInOut' }} />
            </motion.div>
        </div>
    );
